...
class RGCNLayer(nn.Module):
    ...
    def forward(self, g, prev_h=[]):
        ...
        # print(len(prev_h))
        if len(prev_h) != 0 and self.skip_connect:   # 两次计算loop_message的方式不一样，前者激活后再加�?
...
class UnionRGCNLayer(nn.Module):
    ...
    def msg_func(self, edges):
        ...
        # 这里计算的是每个节点发出的消息，节点发出消息时其作为头实�?        # msg = torch.cat((node, relation), dim=1)
        msg = node + relation
        # calculate the neighbor message with weight_neighbor
...
class  ResUnionRGCNLayer(nn.Module):
    def __init__(self, in_feat, out_feat, num_rels, num_bases=-1,  bias=None,
                 activation=None, self_loop=False, dropout=0.0, skip_connect=False, rel_emb=None):
        super(ResUnionRGCNLayer, self).__init__()
        ...
        self.ob = None
        self.sub = None
        self.submat_in = in_feat // num_bases
        self.submat_out = out_feat // num_bases
        # WL
        self.weight_neighbor = nn.Parameter(torch.Tensor(self.in_feat, self.out_feat))
        nn.init.xavier_uniform_(self.weight_neighbor, gain=nn.init.calculate_gain('relu'))

        self.weight_neighbor2 = nn.Parameter(torch.Tensor(self.in_feat, self.out_feat))
        nn.init.xavier_uniform_(self.weight_neighbor2, gain=nn.init.calculate_gain('relu'))

        self.weight_neighbor3 = nn.Parameter(torch.Tensor(self.in_feat, self.out_feat))
        nn.init.xavier_uniform_(self.weight_neighbor3, gain=nn.init.calculate_gain('relu'))
    ...
    def propagate(self, g):
        g.update_all(lambda x: self.msg_func(x), fn.sum(msg='msg', out='h'), self.apply_func2)#Liu 20230619
        #g.update_all(lambda x: self.msg_func(x), self.reduce_entity_attention_func, self.apply_func)


    def reduce_entity_attention_func(self,nodes):
        batch,deep,dim = nodes.mailbox['msg'].shape
        relation_emb = nodes.mailbox['msg'].view(-1,dim)
        att_relation_emb = self.attn_fc(relation_emb)
        att_relation_emb = att_relation_emb.view(batch,deep,1)
        att_relation_emb = F.softmax(att_relation_emb, dim=1)
        return {'h': torch.sum(att_relation_emb * nodes.mailbox['msg'], dim=1)}
    ...
    def forward(self, g, prev_h, emb_rel):
        ...
        g.ndata['h'] = node_repr#论文中 the structural encoder中经过relu的函数表达，g已经改变
    ...
    def msg_func(self, edges):
        ...
        relation = self.rel_emb.index_select(0, edges.data['type']).view(-1, self.out_feat)
        ob_node = edges.src['h'].view(-1, self.out_feat)
        msg = torch.mm(ob_node, self.weight_neighbor) + torch.mm(relation, self.weight_neighbor2) + ob_node  +  relation #论文中 the structural encoder的部分
        #msg = torch.mm(ob_node + relation, self.weight_neighbor)
        return {'msg': msg}


    def apply_func(self, nodes):
        #print(nodes.data['norm'])
        #return {'h': nodes.data['h'] * nodes.data['norm']}
        return {'h': nodes.data['h']}
    def apply_func2(self, nodes):
        #print(nodes.data['norm'])
        return {'h': nodes.data['h'] * nodes.data['norm']}
        #return {'h': nodes.data['h']}