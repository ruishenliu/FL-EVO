...
class RGCNLinkDatasetADV(object):
    ...
    num_rels: int
        number of relations (including reverse relation) of knowledge base
    train: numpy.array
        all relation triplets (src_no_r, rel, dst) for training
    valid: numpy.array
        all relation triplets (src_no_r, rel, dst) for validation
    test: numpy.array
        all relation triplets (src_no_r, rel, dst) for testing
    ...
    def load(self, load_time=True):
        ...
        adv_train_path = os.path.join(self.dir, 'train_data.txt')
        entity_dict = _read_dictionary(entity_path)
        relation_dict = _read_dictionary(relation_path)
        self.train = np.array(_read_triplets_as_list(train_path, entity_dict, relation_dict, load_time))
        self.adv_train = np.array(_read_triplets_as_list(adv_train_path, entity_dict, relation_dict, load_time))
...
def load_from_local_adv(dir, dataset):
    data = RGCNLinkDatasetADV(dataset, dir)
    # if "-d" in dataset:
    #     data.load(load_time=True)
    # else:
    #
